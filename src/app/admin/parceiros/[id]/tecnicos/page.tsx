...
export default function AdminPartnerTechniciansPage() {
  const params = useParams();
  const companyId = params.id as string;
  
  const { data: session, mutate } = useSWR(companyId ? `/api/technicians?companyId=${companyId}` : null, fetcher);
  ...
  if (authSession.status === 'loading' || !authSession.data) return null;
    ...
    <div className="min-h-screen bg-background text-foreground font-sans p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        <header className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          ...
          <div className="flex gap-3">
            <Link href="/admin/parceiros" className="flex items-center gap-2 bg-card border border-border hover:border-primary/50 hover:bg-primary/5 text-foreground px-5 py-2.5 rounded-lg font-semibold transition-all shadow-sm">
              ...
              <span className="hidden sm:inline">Voltar para Parceiros</span>
            </Link>
            <UserMenu />
          </div>
        </header>
        ...
        <div className="bg-card border border-border rounded-xl shadow-lg p-6 mb-8 flex justify-between items-center">
          <div>
            <h2 className="text-xl font-bold">Equipe Técnica Vinculada</h2>
            <p className="text-muted-foreground text-sm">Contas de acesso dos técnicos desta empresa parceira.</p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {!session ? (
                  ...
                  <tr>
                    <td colSpan={5} className="p-8 text-center text-muted-foreground">Este parceiro ainda não possui técnicos cadastrados.</td>
                  </tr>
                ) : (
      ...
      {isCreating && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-2xl shadow-2xl max-w-md w-full p-6 animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-muted-foreground mb-6">Crie uma conta de acesso para um funcionário desta empresa.</p>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                ...
                <Input type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} required placeholder="joao@empresa.com.br" />
              </div>
              <div>
